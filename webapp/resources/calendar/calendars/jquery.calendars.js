/* http://keith-wood.name/calendars.html
   ...
   Please attribute the author if you use it. */
			...
			window.parseDigits = function (string, digits) {
				if (digits) {
					var digitRegExp = new RegExp("[" + digits.join("") + "]", "g");
					return string.replace(digitRegExp, function (w) {
						return getIndexOf(digits,w);
					});
				} else {
					return string;
				}
			...
			window.convertDigits = function (string, digits) {
				if (digits) {
			...
			window.getIndexOf = function(obj, elem){
					var key = -1;
	...
	$.extend(BaseCalendar.prototype, {
		...
		_add: function(date, offset, period) {
			...
			try {
				...
				var m = date.monthOfYear() + (period === 'MM' ? offset : 0);
