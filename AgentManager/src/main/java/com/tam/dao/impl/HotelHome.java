...
public class HotelHome {
	...
	public Hotel merge(Hotel detachedInstance) {
		...
		try {
			Hotel result = (Hotel) sessionFactory.getCurrentSession().merge(detachedInstance);
	...
	public Hotel findById(java.lang.Integer id) {
		...
		try {
			Hotel instance = (Hotel) sessionFactory.getCurrentSession().get("Hotel", id);
			if (instance == null) {
	...
	public List findByExample(Hotel instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("Hotel").add(Example.create(instance))
					.list();
