...
public class CouponHome {
	...
	private static final Log log = LogFactory.getLog(CouponHome.class);
	...
	public Coupon merge(Coupon detachedInstance) {
		...
		try {
			Coupon result = (Coupon) sessionFactory.getCurrentSession().merge(detachedInstance);
	...
	public Coupon findById(java.lang.Integer id) {
		...
		try {
			Coupon instance = (Coupon) sessionFactory.getCurrentSession().get("Coupon", id);
			if (instance == null) {
	...
	public List findByExample(Coupon instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("Coupon").add(Example.create(instance))
					.list();
