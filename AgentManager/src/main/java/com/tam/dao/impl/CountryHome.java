...
public class CountryHome {
	...
	public Country findById(java.lang.String id) {
		...
		try {
			Country instance = (Country) sessionFactory.getCurrentSession().get("Country", id);
			if (instance == null) {
	...
	public List findByExample(Country instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("Country").add(Example.create(instance))
					.list();
