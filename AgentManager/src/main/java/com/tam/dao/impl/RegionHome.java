...
public class RegionHome {
	...
	public Region merge(Region detachedInstance) {
		...
		try {
			Region result = (Region) sessionFactory.getCurrentSession().merge(detachedInstance);
	...
	public Region findById(java.lang.Integer id) {
		...
		try {
			Region instance = (Region) sessionFactory.getCurrentSession().get("Region", id);
			if (instance == null) {
	...
	public List findByExample(Region instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("Region").add(Example.create(instance))
					.list();
