...
public class TicketHome {
	...
	public Ticket findById(java.lang.String id) {
		...
		try {
			Ticket instance = (Ticket) sessionFactory.getCurrentSession().get("Ticket", id);
			if (instance == null) {
	...
	public List findByExample(Ticket instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("Ticket").add(Example.create(instance))
					.list();
