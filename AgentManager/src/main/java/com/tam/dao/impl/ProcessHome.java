...
/**
 * Home object for domain model class Process.
 * @see .Process
 * @author Hibernate Tools
 */
public class ProcessHome {

	private static final Log log = LogFactory.getLog(ProcessHome.class);
	...
	public void persist(Process transientInstance) {
	...
	public Process findById(java.lang.String id) {
		...
		try {
			Process instance = (Process) sessionFactory.getCurrentSession().get("Process", id);
			if (instance == null) {
	...
	public List findByExample(Process instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("Process").add(Example.create(instance))
					.list();
