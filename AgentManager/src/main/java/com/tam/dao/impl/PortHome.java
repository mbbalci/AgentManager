...
/**
 * Home object for domain model class Port.
 * @see .Port
 * @author Hibernate Tools
 */
public class PortHome {

	private static final Log log = LogFactory.getLog(PortHome.class);
	...
	public void persist(Port transientInstance) {
		log.debug("persisting Port instance");
		try {
	...
	public Port merge(Port detachedInstance) {
		...
		try {
			Port result = (Port) sessionFactory.getCurrentSession().merge(detachedInstance);
	...
	public Port findById(java.lang.String id) {
		log.debug("getting Port instance with id: " + id);
		try {
			Port instance = (Port) sessionFactory.getCurrentSession().get("Port", id);
			if (instance == null) {
	...
	public List findByExample(Port instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("Port").add(Example.create(instance))
					.list();
